...
//
//  Changed by Peng Guo on 15. 8. 28..
//  To support controlling by Apple Watch
...
@interface Scene () <WCSessionDelegate>
...
- (void)session:(WCSession *)session didReceiveMessage:(NSDictionary<NSString *,id> *)message replyHandler:(void (^)(NSDictionary<NSString *,id> * _Nonnull))replyHandler
{
    ...
    if ([operation isEqualToString:@"bounce"])
    {
        NSMutableDictionary *replyMessage = [NSMutableDictionary dictionary];
        
        Scene __weak *weakSelf = self;
        
        dispatch_async(dispatch_get_main_queue(), ^{
            
            Scene *strongSelf = weakSelf;
            
            if(wasted){
                [strongSelf startGame];
                [replyMessage setObject:@"started" forKey:@"Reply"];
            }else{
                if (!bird.physicsBody) {
                    [bird startPlaying];
                    if([strongSelf.delegate respondsToSelector:@selector(eventPlay)]){
                        [strongSelf.delegate eventPlay];
                    }
                }
                [bird bounce];
                [replyMessage setObject:@"bounced" forKey:@"Reply"];
            }
